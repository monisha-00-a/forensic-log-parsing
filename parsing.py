...
def generate_timeline(logdir):
    ...
    with open("logs.csv", "w", newline="", encoding="utf-8") as file:
        writer = csv.DictWriter(file, fieldnames=["timestamp", "logs"])
        writer.writeheader()
        for row in invalid_entries:
            writer.writerow({
                "timestamp": row.get("timestamp", ""),
                "logs": row.get("invalid", "")
            })
    print(" logs.csv generated (invalid lines).")
...
def generate_visuals():
    ...
    print(" event_frequency.png saved.")
    ...
    plt.figure(figsize=(10, 5))
    plt.bar(user_counts["user"], user_counts["event_count"], color='purple')
    ...
    if anomalies_df.empty:
        print("ℹ No anomalies found, skipping highlight.")
    ...
    print(" anomalies_highlighted.png saved.")
...
def main():
    ...
    parser.add_argument("logdir", nargs="?", default=os.getcwd(),
                        help="Directory with .vlog files (default: current directory)")
